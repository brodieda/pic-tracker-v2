...
export default function GlobalSearch({ onOpenPic }) {
  ...
  if (!open) {
    return (
      <button
        ...
        title="Search (/)"
        aria-label="Search"
        className="inline-flex items-center justify-center w-9 h-9 rounded-md bg-ink-800 border border-ink-700 hover:border-ink-500 text-ink-300 hover:text-ink-100 transition"
      >
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex sm:items-start sm:justify-center bg-ink-950/70 sm:pt-24">
      ...
      <div className="relative w-full h-full sm:h-auto sm:max-h-[70vh] sm:w-[560px] bg-ink-900 sm:rounded-xl sm:border sm:border-ink-700 shadow-2xl flex flex-col overflow-hidden">
