...
export default function PicCard({ pic, events, eventCfg, onClick }) {
  ...
  // Combine substances + Other free-text for display
  const subsDisplay = [
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-3">
            <h3 className="font-display font-bold text-lg truncate">{pic.name}</h3>
          ...
          <div className="mt-1.5 flex items-center gap-2 flex-wrap">
            ...
            {pic.gender && (
              <span className="text-xs text-ink-400">{pic.gender}{pic.ageRange ? ` · ${pic.ageRange}` : ''}</span>
            )}
          </div>
          ...
          {subsDisplay.length > 0 && (
            <div className="mt-3">
              <p className="text-[10px] font-display tracking-[0.2em] uppercase text-ink-500 mb-1">Substances</p>
          ...
          {presDisplay.length > 0 && (
            <div className="mt-2">
              <p className="text-[10px] font-display tracking-[0.2em] uppercase text-ink-500 mb-1">Presentations</p>
              <div className="flex flex-wrap gap-1.5">
                {presDisplay.map((p, i) => (
                  <span
                    key={`${p}-${i}`}
